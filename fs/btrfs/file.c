				    ...
				    struct iov_iter *from)
{
	...
	ret = dax_iomap_rw(iocb, from, &btrfs_iomap_ops);
				      ...
				      struct iov_iter *from)
{
	...
	/*
	 * Use pos instead of round_down(pos, fs_info->sectorsize) to ensure that
				      ...
				      struct vm_fault *vmf)
{
	...
	if (IS_DAX(inode))
		ret = dax_iomap_fault(vma, vmf, &btrfs_iomap_ops);
	else

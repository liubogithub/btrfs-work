...
static int scrub_extent(struct scrub_ctx *sctx, struct map_lookup *map, u64 logical, u64 len,
			u64 physical, struct btrfs_device *dev, u64 flags,
				 ...
				 u64 logical)
{
	...
	if (test_bit(EXTENT_FLAG_PREALLOC, &em->flags))
		trace_printk("a prealloc em inode %llu\n", btrfs_ino(inode));
...
static noinline int write_page_nocow(struct scrub_ctx *sctx,
			    u64 physical_for_dev_replace, struct page *page)
{
	struct bio *bio;
	struct btrfs_device *dev;
